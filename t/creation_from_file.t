...
$tests_ran++;
eval {
	$object = Regexp::F2B::INI->load( file => 't/filter.d/fast-log-attack-src.conf' );
	$object = Regexp::F2B::INI->load( file => 't/filter.d/sshd.conf' );
	$worked = 1;
};
...
$tests_ran++;
eval {
	$object = Regexp::F2B::INI->load( file => 't/filter.d/fast-log-attack-src.conf-does-not-exist' );
	$worked = 1;
};
...
$tests_ran++;
my $test
	= '12/08/2022-21:44:56.542761  [**] [1:2402000:6468] ET DROP Dshield Block Listed Source group 1 [**] [Classification: Misc Attack] [Priority: 2] {TCP} 205.210.31.54:53983 -> 192.168.14.42:8';
my $expected = '205.210.31.54';
eval {
	$object = Regexp::F2B::INI->load( file => 't/filter.d/fast-log-attack-src.conf' );
	my $found = $object->proc_line($test);
	if ( $found->{data}{HOST} ne $expected ) {
		die(      '$object->proc_lines($test) did not return the expected "'
				. $expected
				. '" for "'
				. $test
				. '"... got... '
...
$worked = 0;
$tests_ran++;
$test = 'Dec 18 23:03:44 vixen42 sshd[14346]: error: PAM: Authentication error for illegal user foo from 205.210.31.54';
